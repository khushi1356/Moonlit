...
const ReviewSection = ({ serviceId, serviceName }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!token || !user) { toast.error('Please login to leave a review.'); return; }
  ...
  return (
    <div className="mt-8 pt-8 border-t border-[var(--color-primary)]/10">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => { if (!token || !user) { toast.error('Please login to leave a review.'); return; } setShowForm(!showForm); }}
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button type="button" onClick={() => setShowForm(false)} className="text-gray-400 hover:text-[var(--color-primary)]"><ChevronUp className="w-5 h-5 rotate-180" /></button>
              </div>
              <p className="text-xs tracking-widest uppercase font-bold text-gray-400 mb-4">Your Rating</p>
              <StarRating value={rating} onChange={setRating} />
              <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder={`Share your experience with ${serviceName}...`} rows={3} className="w-full mt-4 p-4 border border-[var(--color-primary)]/20 bg-transparent text-[var(--color-primary)] text-sm focus:border-[var(--color-rose)] outline-none resize-none font-sans rounded-lg" />
              <button type="submit" disabled={submitting} className="mt-6 w-full flex justify-center items-center gap-2 px-6 py-3 bg-[var(--color-primary)] text-white text-xs font-bold tracking-widest uppercase rounded-lg hover:bg-[var(--color-primary-light)] transition-colors disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {reviews.map((r) => (
            <motion.div key={r._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-4">
